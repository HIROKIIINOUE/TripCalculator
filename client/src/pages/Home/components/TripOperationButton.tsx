import { IoMdAdd } from "react-icons/io"
import { MdDeleteOutline } from "react-icons/md";
import { useAuth } from "../../../contexts/auth/useAuth";
import toast from "react-hot-toast";
import type { Trip } from "../../../types/trip.type";
import { useTranslation } from "react-i18next";

type Props = {
  trips: Trip[],
  setTrips: React.Dispatch<React.SetStateAction<Trip[]>>
  selectedTripIds: number[]
  setSelectedTripIds: React.Dispatch<React.SetStateAction<number[]>>
  setIsAddModalOpen: React.Dispatch<React.SetStateAction<boolean>>
}

const TripOperationButton = (props: Props) => {
  const { accessToken } = useAuth()
  const { t } = useTranslation("home")
  const { setTrips, selectedTripIds, setSelectedTripIds, setIsAddModalOpen } = props

  const handleDelete = async () => {
    if (!confirm(t("home.operation.deleteConfirm"))) return
    const BACKEND_URL = import.meta.env.VITE_API_BASE_URL

    try {
      const results = await Promise.all(
        selectedTripIds.map(async (tripId) => {
          const res = await fetch(`${BACKEND_URL}/trips/${tripId}`, {
            method: "DELETE",
            headers: {
              "authorization": `Bearer ${accessToken}`
            }
          })
          return {
            tripId,
            ok: res.ok,
            status: res.status
          }
        })
      )
      const deletedIds = results.filter(result => result.ok).map(result => result.tripId)
      setTrips(prev => prev.filter(trip => {
        const judge = deletedIds.includes(trip.id)
        return judge === false
      }))

      if (deletedIds.length === selectedTripIds.length) {
        toast.success(t("home.operation.deleteSuccess"))
      } else if (deletedIds.length > 0) {
        toast.error(t("home.operation.deletePartialFailure"))
      } else {
        toast.error(t("home.operation.deleteFailure"))
      }
      setSelectedTripIds([])
    } catch (error) {
      console.error(error)
      toast.error(t("home.networkError"))
    }

  }

  return (
    <>
      {selectedTripIds.length > 0 ? (
        <button
          type="button"
          onClick={handleDelete}
          className="inline-flex items-center justify-center gap-2 rounded-full border border-red-200 bg-orange-800 px-5 py-2.5 text-sm font-bold text-white shadow-[0_18px_34px_-18px_rgba(220,38,38,0.95)] transition hover:scale-[1.01] hover:from-red-600 hover:to-red-700 focus:outline-none cursor-pointer"
        >
          <MdDeleteOutline className="text-lg" />
          {t("home.operation.deleteTrip")}
        </button>
      ) : (
        <button
          type="button"
          onClick={() => setIsAddModalOpen(true)}
          className="inline-flex items-center justify-center gap-2 rounded-full bg-orange-500 px-4 py-2.5 text-sm font-semibold text-white shadow-[0_14px_30px_-18px_rgba(249,115,22,0.95)] transition hover:bg-orange-600 focus:outline-none cursor-pointer"
        >
          <IoMdAdd className="text-lg" />
          {t("home.operation.addTrip")}
        </button>
      )}

    </>

  )
}

export default TripOperationButton
